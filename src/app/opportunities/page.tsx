            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '2rem' }}>
                {opportunities.map((opp) => (
                    <motion.div
                        ...
                        whileTap={{ scale: 0.98 }}
                    >
                        <Card variant="elevated" style={{ display: 'flex', flexDirection: 'column', height: '100%', pointerEvents: 'none' }}>
                            <div style={{ marginBottom: '1rem' }}>
                                ...
                                <motion.h2 style={{ fontSize: '1.25rem', marginBottom: '0.5rem', lineHeight: 1.4 }}>{opp.title}</motion.h2>
                                <motion.p style={{ fontSize: '0.875rem', color: 'var(--md-sys-color-secondary)', fontWeight: 500 }}>
                                    {opp.organization}
                                </motion.p>
                            </div>
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '1.5rem' }}>
                                ...
                                {opp.skills.length > 3 && (
                                    <span style={{ fontSize: '0.75rem', padding: '2px 8px', color: 'var(--md-sys-color-secondary)' }}>
                            ...
                            <div style={{ marginTop: 'auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <span style={{ fontSize: '0.8rem', color: 'var(--md-sys-color-secondary)' }}>
                                    Deadline: {opp.deadline}
                                </span>
                                <Button variant="glass" style={{ pointerEvents: 'auto', fontSize: '0.9rem', fontWeight: 600 }}>
                                    Read More
                                </Button>
                            </div>
                        </Card>
                ...
                {selectedId && (
                    <>
                        ...
                        <div style={{
                            ...
                            {opportunities.filter(item => item.id === selectedId).map(opp => (
                                <motion.div
                                    layoutId={selectedId}
                                    key={opp.id}
                                    style={{
                                ...
                                >
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '1rem', borderTop: '1px solid var(--md-sys-color-outline-variant)' }}>
                                        <span style={{ color: 'var(--md-sys-color-secondary)' }}>
                                            Deadline: <strong>{opp.deadline}</strong>
                                        </span>
                                        <Button variant="filled" style={{ padding: '12px 24px', fontSize: '1rem' }}>
                                            Apply Now
                                        </Button>
                                    </div>
                                </motion.div>
